            ...
            filteredAppointments.map((appt) => (
              <div key={appt.id} className="bg-white rounded-lg shadow-sm border border-gray-200 p-4">
                <div className="flex items-start justify-between mb-3">
                  <div className="flex-1">
                    <h3 className="text-sm font-bold text-gray-900">{appt.customer_name}</h3>
                    <p className="text-xs text-gray-500 mt-1">{getLocalizedServiceName(appt)}</p>
                  </div>
                  <Button
                    variant="ghost"
                    size="sm"
                ...
                <div className="space-y-2 text-sm">
                  <div className="flex justify-between">
                    <span className="text-gray-500">{t('appointments.artist')}:</span>
                    <span className="font-medium text-gray-900">{appt.artist_name || 'N/A'}</span>
                  </div>
                  <div className="flex flex-col gap-1">
                    <span className="text-gray-500">{t('appointments.contact')}:</span>
                    <span className="text-gray-900">{appt.customer_email}</span>
                    <span className="text-gray-600">{appt.customer_phone}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-500">{t('appointments.dateTime')}:</span>
                    <div className="text-right">
                      <div className="font-medium text-gray-900">{appt.appointment_date}</div>
                      <div className="text-gray-600">{appt.appointment_time}</div>
                    </div>
                  </div>
                </div>
                <div className="mt-4 pt-3 border-t border-gray-200">
                  <label className="block text-xs text-gray-500 mb-2">{t('appointments.status')}:</label>
                  <Select
                    value={appt.status}
                    onValueChange={(value) => handleStatusChange(appt.id, value)}
                  >
