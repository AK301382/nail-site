    ...
    <AdminLayout>
      <div className="space-y-8">
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4 md:p-6">
          <h2 className="text-lg md:text-xl font-bold mb-4" style={{ color: '#8B6F8E' }}>{t('dashboard.recentAppointments')}</h2>
          <div className="space-y-3">
            {recentAppointments.length === 0 ? (
              <p className="text-gray-500 text-center py-8">{t('dashboard.noAppointments')}</p>
            ) : (
              recentAppointments.map((appt) => (
                <div key={appt.id} className="flex flex-col sm:flex-row sm:items-center sm:justify-between p-3 md:p-4 bg-gray-50 rounded-lg gap-2 sm:gap-0">
                  <div className="flex-1">
                    <p className="font-medium text-sm md:text-base">{appt.customer_name}</p>
                    <p className="text-xs md:text-sm text-gray-600">{appt.customer_email}</p>
                  </div>
                  <div className="flex items-center justify-between sm:justify-end sm:flex-col sm:items-end gap-2">
                    <p className="text-xs md:text-sm font-medium">{appt.appointment_date}</p>
                    <span 
